<x-layouts.app
    ...
    description="Create a workout plan for a client."
    :breadcrumbs="[['label' => 'Workout Plans', 'url' => route('workouts.index')], ['label' => 'New Workout Plan']]">

    <form method="POST" action="{{ route('workouts.store') }}" x-data="workoutForm()">
        @csrf
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            <div class="space-y-6 lg:col-span-2">
                <x-card title="Plan Details">
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2">
                            <x-input label="Plan name" name="name" value="{{ old('name') }}" required placeholder="e.g. 12-Week Strength" />
                        </div>
                        <div class="sm:col-span-2">
                            <x-input label="Goal" name="goal" value="{{ old('goal') }}" placeholder="e.g. Build muscle, lose fat" />
                        </div>
                        <x-input label="Start date" type="date" name="start_date" value="{{ old('start_date', now()->toDateString()) }}" />
                        <x-input label="End date" type="date" name="end_date" value="{{ old('end_date') }}" />
            ...
            <div class="space-y-6">
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('workouts.index') }}" class="btn-outline">Cancel</a>
                </div>
            </div>
        </div>
    </form>
